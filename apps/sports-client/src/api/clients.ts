...
clientsApiAxios.interceptors.response.use(
	(response) => response,
	(error) => {
		if (error.response.status < 401 && error.response.status > 300) {
...
export const fetchAllClients = createAsyncThunk('client/fetchAll', async (clubId: string | undefined) => {
	const response = await clientsApiAxios.get<IClient[]>(`${clubId}/clients`);
	return response.data;
});

export const fetchClient = createAsyncThunk('client/fetchOne', async (client: IClient) => {
	const { id, clubId } = client;
	const response = await clientsApiAxios.get<IClient>(`${clubId}/clients/${id}`);
	return response.data;
});

export const createClient = createAsyncThunk('client/createClient', async (client: CreateClient) => {
...
export const updateClient = createAsyncThunk('client/updateClient', async (client: IClient) => {
...
export const removeClient = createAsyncThunk('client/remove', async (client: IClient) => {
	const { clubId, id } = client;
	const response = await clientsApiAxios.delete<IClient>(`${clubId}/clients/${id}`);
	return response.data ? response.data : toast.success('Success Deleted!', {});
});
